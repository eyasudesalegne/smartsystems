...
def build_local_payload(tenant_id: str, service_names: list[str], persist: bool) -> dict:
    ...
    return main_mod._build_connector_credential_matrix(tenant_id=tenant_id, service_names=service_names, persist=persist)


def build_remote_payload(base_url: str, tenant_id: str, service_names: list[str], persist: bool, timeout: int) -> dict:
    url = base_url.rstrip('/') + '/connectors/credential-matrix'
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Build a connector credential matrix from the local package or a running service.')
    ...
    parser.add_argument('--out', default=str(ROOT / 'docs' / 'generated_connector_credential_matrix.json'))
    ...
    print(json.dumps({
        ...
        'unique_credential_key_count': payload.get('unique_credential_key_count', 0),
    }))
    return 0
