...
def build_local_payload(tenant_id: str, service_names: list[str], persist: bool) -> dict:
    ...
    return main_mod._build_connector_deployment_plan(tenant_id=tenant_id, service_names=service_names, persist=persist)


def build_remote_payload(base_url: str, tenant_id: str, service_names: list[str], persist: bool, timeout: int) -> dict:
    url = base_url.rstrip('/') + '/connectors/deployment-plan'
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Build a connector deployment plan from the local package or a running service.')
    ...
    parser.add_argument('--out', default=str(ROOT / 'docs' / 'generated_connector_deployment_plan.json'))
