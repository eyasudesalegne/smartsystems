...
def build_local_payload(tenant_id: str, service_names: list[str], persist: bool) -> dict:
    ...
    return main_mod._build_connector_rollout_bundle(tenant_id=tenant_id, service_names=service_names, persist=persist)


def build_remote_payload(base_url: str, tenant_id: str, service_names: list[str], persist: bool, timeout: int) -> dict:
    url = base_url.rstrip('/') + '/connectors/rollout-bundle'
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Build a combined connector rollout bundle from the local package or a running service.')
    ...
    parser.add_argument('--out', default=str(ROOT / 'docs' / 'generated_connector_rollout_bundle.json'))
    ...
    print(json.dumps({
        ...
        'ready_to_import_count': payload.get('ready_to_import_count', 0),
        'requires_credentials_count': payload.get('requires_credentials_count', 0),
    }))
    return 0
