...
def build_local_payload(tenant_id: str, service_names: list[str], persist: bool) -> dict:
    ...
    return main_mod._build_connector_preflight(tenant_id=tenant_id, service_names=service_names, persist=persist)


def build_remote_payload(base_url: str, tenant_id: str, service_names: list[str], persist: bool, timeout: int) -> dict:
    url = base_url.rstrip('/') + '/connectors/preflight'
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Generate a connector preflight report from the local package or a running service.')
    ...
    parser.add_argument('--out', default=str(ROOT / 'docs' / 'generated_connector_preflight_report.json'))
    ...
    print(json.dumps({'status': 'ok', 'out': str(out_path), 'count': payload.get('count', 0), 'configured_count': payload.get('configured_count', 0), 'live_ready_count': payload.get('live_ready_count', 0)}))
    return 0
